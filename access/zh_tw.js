...
function translateBody(fobj) {
	...
	for (var i = 0; i < objs.length; i++) {
		...
		if (obj.tagName == "INPUT" && obj.value != "" && obj.type != "text" && obj.type != "hidden") obj.value =
			translateText(obj.value);
	...
	self.count = function(timestamp) {
		...
		if (self.options.useEasing) {
			if (self.countDown) {
				...
				self.frameVal = self.options.easingFn(progress, self.startVal, self.endVal - self.startVal, self
					.duration)
			}
		} else {
			if (self.countDown) {
